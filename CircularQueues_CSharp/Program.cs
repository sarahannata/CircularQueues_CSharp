...
namespace CircularQueues_CSharp
{
    class Queues
    {
        ...
        public void insert(int element)
        {
            ...
            {
                /* If the REAR is at the last position of the array, than the value of REAR is set to 0 that corresponds to the first position of the array. */
                if (REAR == max -1) REAR = 0;
                else
                    /* If REAR is not at the last position, then its value is incremented by one. */
                    REAR = REAR + 1;    
            }
            /* Once the posiiton of REAR is determined, the element is added at its proper place. */
